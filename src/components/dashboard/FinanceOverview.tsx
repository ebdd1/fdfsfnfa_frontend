...
export const FinanceOverview: React.FC<FinanceOverviewProps> = ({ properties, formatPrice }) => {
  ...
  if (properties.length === 0) {
    return (
      <div className="bg-white rounded-[24px] shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-slate-100/50 p-6 sm:p-12 flex flex-col items-center justify-center text-center animate-in fade-in duration-300">
  ...
  return (
    <div className="space-y-4 sm:space-y-6 pb-20 md:pb-6 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
        ...
        <div className="relative overflow-hidden bg-white rounded-[24px] p-4 sm:p-5 shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-slate-100/50">
          ...
          <div className="relative flex items-center justify-between mb-4 sm:mb-6">
            ...
            <button className="text-slate-400 hover:bg-slate-50 p-1 rounded-md cursor-pointer" aria-label="Menu Opsi Keuangan">
          ...
          <div className="relative mb-3 sm:mb-4">
            ...
            <p className="text-xs text-slate-500 mt-1">per bulan, dari {activeTenants} sewa aktif</p>
          </div>
          ...
          <div className="relative flex items-center gap-2 text-xs">
            ...
            <span className="text-slate-500">diperbarui otomatis saat sewa berubah</span>
          </div>
        </div>

        {/* Breakdown card */}
        <div className="bg-white rounded-[24px] p-4 sm:p-5 shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-slate-100/50 flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-2 text-slate-600 font-medium text-sm mb-4 sm:mb-6">
              <div className="bg-[var(--primary-50)] p-1.5 rounded-lg">
            ...
            <div className="space-y-4 sm:space-y-5">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-slate-700">Penyewa Aktif</p>
                    <p className="text-xs text-slate-500">{activeTenants} sewa berjalan</p>
                  </div>
                </div>
                <span className="text-sm font-bold text-slate-800">{formatPrice(monthlyRevenue)}</span>
              </div>

              <div className="flex items-center justify-between border-t border-slate-100 pt-4 sm:pt-5">
                <div className="flex items-center gap-2.5">
                  <div className="bg-amber-50 p-2 rounded-lg">
                    <Clock className="w-4 h-4 text-amber-500" />
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-slate-700">Menunggu Pembayaran</p>
                    <p className="text-xs text-slate-500">{awaitingOrders.length} order belum dibayar</p>
                  </div>
                </div>
                <span className="text-sm font-semibold text-amber-700">{formatPrice(awaitingAmount)}</span>
              </div>
              ...
              <div className="flex items-center justify-between border-t border-slate-100 pt-4 sm:pt-5">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-slate-700">Potensi Pendapatan</p>
                    <p className="text-xs text-slate-500">{availableRooms.length} kamar tersedia</p>
                  </div>
                </div>
                <span className="text-sm font-semibold text-slate-600">+ {formatPrice(potentialRevenue)}</span>
              </div>
            </div>
          </div>

          <div className="mt-4 sm:mt-6 flex items-start gap-2 bg-slate-50 text-slate-600 p-3 rounded-xl text-xs font-medium border border-slate-100">
            <TrendingUp className="w-3.5 h-3.5 shrink-0 mt-0.5 text-slate-500" />
